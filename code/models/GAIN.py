...
#             print("="*100)
#         self.predict = [nn.Sequential(
#             nn.Linear(self.bank_size * 5//self.num_split*i + self.gcn_dim * 4, self.bank_size * 2//self.num_split*i),
#             self.activation,
#             self.dropout,
#             nn.Linear(self.bank_size * 2//self.num_split*i, config.relation_nums),
#         )for i in range(1, self.num_split + 1)]
...
#         print(self.path_info_mapping)
    ...
    def Inception_module(self, graphs, output_feature, params, bsz, idx):
        bank_size = len(output_feature[0])

        entity2mention_table = params['entity2mention_table']  # list of [entity_num, mention_num]
        entity_num = torch.max(params['entity_id'])
        entity_bank = get_cuda(torch.Tensor(bsz, entity_num, bank_size))
        global_info = get_cuda(torch.Tensor(bsz, bank_size))
        ...
        # [batch_size, h_t_limit, bank_size]
        h_entity_index = h_t_pairs[:, :, 0].unsqueeze(-1).expand(-1, -1, bank_size)
        t_entity_index = h_t_pairs[:, :, 1].unsqueeze(-1).expand(-1, -1, bank_size)
